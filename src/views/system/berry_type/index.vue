<template>
  <div v-loading="loading" class="app-container">
    <el-card>
      <div slot="header">
        <span> 任务类型列表({{ obj.total }}) </span>
        <el-button v-permission="[url.createBerryTypeUrl]" icon="el-icon-plus" type="text" size="small" style="float: right; padding: 3px 0;" @click="handleCreateObj">
          添加
        </el-button>
      </div>
      <div>
        <el-table :data="obj.dataList" style="width: 100%">
          ...
          >
            <template slot-scope="{row}">
              <router-link
                :to="{name: 'BerryTypeDetail', params:{ id: row.id }}"
              >
                <el-button size="mini" type="text" style="margin-right: 8px"> 查看 </el-button>
              </router-link>
              <el-button v-permission="[url.updateBerryTypeUrl]" size="mini" type="text" style="margin-right: 8px" @click="handleUpdateObj(row)">
                编辑
              </el-button>
              <el-popconfirm title="确定删除?" @onConfirm="deleteObj(row.id)">
                <el-button slot="reference" v-permission="[url.deleteBerryTypeUrl]" size="mini" type="text">
                  删除
                </el-button>
              </el-popconfirm>
            </template>
...
import {
  deleteBerryTypeApi as deleteObjApi,
  getBerryTypeListApi as getObjListApi
