      ...
      <div>
        <el-table :data="obj.dataList" style="width: 100%">
          ...
          <el-table-column fixed="right" label="操作" width="120">
            <template slot-scope="{row}">
              <el-button v-permission="[url.updateRdsAccountUrl]" size="mini" type="text" style="margin-right: 8px" @click="handleUpdateObj(row.id)">
                编辑
              </el-button>
              <router-link
                :to="{name: 'RdsAccountDetail', params:{ id: row.id }}"
              >
...
import {
  getRdsAccountListApi as getObjListApi
} from '@/api/asset/rds'
export default {
  name: 'RdsAccountList',
