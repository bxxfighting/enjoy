<template>
  <div v-loading="loading" class="app-container">
    <el-card>
      <div slot="header">
        <span> JenkinsJob列表({{ obj.total }}) </span>
        <el-button v-permission="[url.syncJenkinsJobUrl]" icon="el-icon-refresh" type="text" size="small" style="float: right; padding: 3px 0;" @click="syncObj">
          同步
        </el-button>
      </div>
      <div>
        <el-table :data="obj.dataList" style="width: 100%">
          ...
          <el-table-column prop="url" label="访问地址">
            <template slot-scope="{row}">
              ...
              <el-link class="el-icon-link" :href="row.url" target="_blank" />
            </template>
          </el-table-column>
        </el-table>
...
import {
  getJenkinsJobListApi as getObjListApi,
  syncJenkinsJobApi as syncObjApi
