  ...
  <div>
    <el-card>
      <div slot="header">
        ...
        <el-button v-permission="[url.createServiceDomainUrl]" icon="el-icon-plus" type="text" size="small" style="float: right; padding: 3px 0;" @click="handleCreateObj">
          添加
        </el-button>
      </div>
      <div>
        <el-table :data="obj.dataList" style="width: 100%">
          ...
          <el-table-column prop="instance_id" label="实例ID">
            <template slot-scope="{row}">
              <CopyField :value="row.domain.instance_id" />
            </template>
          </el-table-column>
          ...
          <el-table-column fixed="right" label="操作" width="160">
            <template slot-scope="{row}">
              ...
              <el-popconfirm title="确定删除?" @onConfirm="deleteObj(row.domain.id)">
                <el-button slot="reference" v-permission="[url.deleteServiceDomainUrl]" size="mini" type="text">
                  删除
                </el-button>
              </el-popconfirm>
            </template>
...
import {
  getServiceDomainListApi as getObjListApi,
  deleteServiceDomainApi as deleteObjApi
