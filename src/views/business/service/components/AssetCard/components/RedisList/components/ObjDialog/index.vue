  ...
  <el-dialog :title="titleMap[status]" :visible.sync="visible" :close-on-click-modal="false" @open="open" @close="$emit('update:show', false)">
    <el-form ref="form" :model="form" :rules="rules" label-width="80px">
      <el-form-item label="Redis" prop="redis">
        <RedisField :obj-id.sync="form.asset_obj_id" :regain.sync="regain" />
      </el-form-item>
      <el-form-item>
...
<script>
import {
  createServiceRedisApi as createObjApi
} from '@/api/business/service'
import RedisField from '@/components/Field/RedisField'
export default {
  name: 'FormDialog',
  components: { RedisField },
  props: {
    serviceId: {
      required: true,
      type: Number
  ...
  data() {
    return {
      ...
      rules: {
        asset_obj_id: [{ required: true, message: '请选择Redis', trigger: 'change' }]
      }
    }
  },
  watch: {
