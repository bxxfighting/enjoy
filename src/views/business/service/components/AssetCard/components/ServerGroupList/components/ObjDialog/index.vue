<template>
  <el-dialog :title="titleMap[status]" :visible.sync="visible" :close-on-click-modal="false" @open="open" @close="$emit('update:show', false)">
    <el-form ref="form" :model="form" :rules="rules" label-width="120px">
      <el-form-item label="SLB服务器组" prop="slb_server_group">
        <SlbServerGroupField :obj-id.sync="form.asset_obj_id" :regain.sync="regain" />
      </el-form-item>
      <el-form-item>
...
<script>
import {
  createServiceServerGroupApi as createObjApi
} from '@/api/business/service'
import SlbServerGroupField from '@/components/Field/SlbServerGroupField'
export default {
  name: 'FormDialog',
  components: { SlbServerGroupField },
  props: {
    serviceId: {
      required: true,
      type: Number
  ...
  data() {
    return {
      ...
      rules: {
        asset_obj_id: [{ required: true, message: '请选择服务器组', trigger: 'change' }]
      }
    }
  },
  watch: {
