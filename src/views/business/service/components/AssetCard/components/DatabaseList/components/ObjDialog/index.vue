  ...
  <el-dialog :title="titleMap[status]" :visible.sync="visible" :close-on-click-modal="false" @open="open" @close="$emit('update:show', false)">
    <el-form ref="form" :model="form" :rules="rules" label-width="120px">
      <el-form-item label="数据库" prop="database">
        <RdsDatabaseField :obj-id.sync="form.asset_obj_id" :regain.sync="regain" />
      </el-form-item>
      <el-form-item>
...
<script>
import {
  createServiceDatabaseApi as createObjApi
} from '@/api/business/service'
import RdsDatabaseField from '@/components/Field/RdsDatabaseField'
export default {
  name: 'FormDialog',
  components: { RdsDatabaseField },
  props: {
    serviceId: {
      required: true,
      type: Number
  ...
  data() {
    return {
      ...
      rules: {
        asset_obj_id: [{ required: true, message: '请选择数据库', trigger: 'change' }]
      }
    }
  },
  watch: {
