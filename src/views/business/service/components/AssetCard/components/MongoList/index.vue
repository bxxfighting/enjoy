        ...
        <el-table :data="obj.dataList" style="width: 100%">
          <el-table-column prop="name" label="名称">
            <template slot-scope="{row}">
              <CopyField :value="row.mongo.name" />
            </template>
          </el-table-column>
          <el-table-column prop="instance_id" label="实例ID">
            <template slot-scope="{row}">
              <CopyField :value="row.mongo.instance_id" />
            </template>
          </el-table-column>
          <el-table-column prop="connection" label="连接字符串">
            <template slot-scope="{row}">
              <CopyField :value="String(row.mongo.connection)" />
            </template>
          </el-table-column>
          <el-table-column prop="version" label="版本">
            <template slot-scope="{row}">
              <CopyField :value="String(row.mongo.version)" />
            </template>
          </el-table-column>
          ...
          <el-table-column fixed="right" label="操作" width="160">
            <template slot-scope="{row}">
              <router-link
                :to="{name: 'MongoDetail', params:{ id: row.mongo.id }}"
              >
                <el-button size="mini" type="text" style="margin-right: 8px"> 查看 </el-button>
              </router-link>
              <el-popconfirm title="确定删除?" @onConfirm="deleteObj(row.mongo.id)">
                <el-button slot="reference" v-permission="[url.deleteServiceMongoUrl]" size="mini" type="text">
                  删除
                </el-button>
              </el-popconfirm>
            </template>
...
import {
  getServiceMongoListApi as getObjListApi,
  deleteServiceMongoApi as deleteObjApi
