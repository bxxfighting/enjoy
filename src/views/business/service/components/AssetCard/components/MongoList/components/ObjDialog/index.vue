<template>
  <el-dialog :title="titleMap[status]" :visible.sync="visible" :close-on-click-modal="false" @open="open" @close="$emit('update:show', false)">
    <el-form ref="form" :model="form" :rules="rules" label-width="80px">
      <el-form-item label="Mongo" prop="mongo">
        <MongoField :obj-id.sync="form.asset_obj_id" :regain.sync="regain" />
      </el-form-item>
      <el-form-item>
...
<script>
import {
  createServiceMongoApi as createObjApi
} from '@/api/business/service'
import MongoField from '@/components/Field/MongoField'
export default {
  name: 'FormDialog',
  components: { MongoField },
  props: {
    serviceId: {
      required: true,
      type: Number
  ...
  data() {
    return {
      ...
      rules: {
        asset_obj_id: [{ required: true, message: '请选择Mongo', trigger: 'change' }]
      }
    }
  },
  watch: {
