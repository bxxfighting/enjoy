<template>
  <div>
    <el-card shadow="never">
      <div slot="header">
        <span> Rocket Topic列表({{ obj.total }}) </span>
        <el-button v-permission="[url.createServiceRocketTopicUrl]" icon="el-icon-plus" type="text" size="small" style="float: right; padding: 3px 0;" @click="handleCreateObj">
          添加
        </el-button>
      </div>
      <div>
        <el-table :data="obj.dataList" style="width: 100%">
          <el-table-column prop="name" label="名称">
            <template slot-scope="{row}">
              <CopyField :value="row.rocket_topic.name" />
            </template>
          </el-table-column>
          <el-table-column prop="rocket" label="所属Rocket">
            <template slot-scope="{row}">
              <router-link
                :to="{name: 'RocketDetail', params:{ id: row.rocket_topic.rocket.id }}"
              >
                <el-button size="mini" type="text" style="margin-right: 8px">{{ row.rocket_topic.rocket.name }}</el-button>
              </router-link>
            </template>
          </el-table-column>
          ...
          <el-table-column fixed="right" label="操作" width="160">
            <template slot-scope="{row}">
              <el-popconfirm title="确定删除?" @onConfirm="deleteObj(row.rocket_topic.id)">
                <el-button slot="reference" v-permission="[url.deleteServiceRocketTopicUrl]" size="mini" type="text">
                  删除
                </el-button>
              </el-popconfirm>
            </template>
...
import {
  getServiceRocketTopicListApi as getObjListApi,
  deleteServiceRocketTopicApi as deleteObjApi
