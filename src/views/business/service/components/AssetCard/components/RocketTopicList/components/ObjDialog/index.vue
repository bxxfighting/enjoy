<template>
  <el-dialog :title="titleMap[status]" :visible.sync="visible" :close-on-click-modal="false" @open="open" @close="$emit('update:show', false)">
    <el-form ref="form" :model="form" :rules="rules" label-width="120px">
      <el-form-item label="Rocket Topic" prop="database">
        <RocketTopicField :obj-id.sync="form.asset_obj_id" :regain.sync="regain" />
      </el-form-item>
      <el-form-item>
...
<script>
import {
  createServiceRocketTopicApi as createObjApi
} from '@/api/business/service'
import RocketTopicField from '@/components/Field/RocketTopicField'
export default {
  name: 'FormDialog',
  components: { RocketTopicField },
  props: {
    serviceId: {
      required: true,
      type: Number
  ...
  data() {
    return {
      ...
      rules: {
        asset_obj_id: [{ required: true, message: '请选择Rocket Topic', trigger: 'change' }]
      }
    }
  },
  watch: {
