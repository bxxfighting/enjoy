<template>
  <el-dialog :title="titleMap[status]" :visible.sync="visible" :close-on-click-modal="false" @open="open" @close="$emit('update:show', false)">
    <el-form ref="form" :model="form" :rules="rules" label-width="80px">
      <el-form-item label="ECS" prop="ecs">
        <EcsField :obj-id.sync="form.asset_obj_id" :regain.sync="regain" />
      </el-form-item>
      <el-form-item>
...
<script>
import {
  createServiceEcsApi as createObjApi
} from '@/api/business/service'
import EcsField from '@/components/Field/EcsField'
export default {
  name: 'FormDialog',
  components: { EcsField },
  props: {
  ...
  data() {
    return {
      ...
      rules: {
        asset_obj_id: [{ required: true, message: '请选择ECS', trigger: 'change' }]
      }
    }
  },
  watch: {
