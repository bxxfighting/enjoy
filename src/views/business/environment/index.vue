  ...
  <div v-loading="loading" class="app-container">
    <el-card>
      <div slot="header">
        ...
        <el-button v-permission="[url.createEnvironmentUrl]" icon="el-icon-plus" type="text" size="small" style="float: right; padding: 3px 0;" @click="handleCreateObj">
          添加
        </el-button>
      </div>
      <div>
        <el-table :data="obj.dataList" style="width: 100%">
          ...
          >
            <template slot-scope="{row}">
              <el-button v-permission="[url.updateEnvironmentUrl]" size="mini" type="text" style="margin-right: 8px" @click="handleUpdateObj(row)">
                编辑
              </el-button>
              <el-popconfirm v-permission="[url.deleteEnvironmentUrl]" title="确定删除?" @onConfirm="deleteObj(row.id)">
...
import url from '@/api/business/environment/url'
import {
  deleteEnvironmentApi as deleteObjApi,
  getEnvironmentListApi as getObjListApi
} from '@/api/business/environment'
